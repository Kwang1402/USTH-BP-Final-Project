void table(FILE *fp, student *students, int n) {
    ...
    for(int i = 1; i <= n; ++i){
        fprintf(fp, "\n%-10s \t%-10s \t%-20s \t%d/%d/%d \t%-10.2f \t%-10.2f \t%-10.2f \t%-10.2f \n",
                ...
                students[i].grade[0], students[i].grade[1], students[i].grade[2],gpa(students[i])
                );
    }
...
void output(student *students, int n) {
    ...
    //split the fullnames into lastnames and firstnames
    nameSplit(students, n);

    //print student list as a table to screen and a text file "StudentList.txt" 
    //3rd task and 4th task
